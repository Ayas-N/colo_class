...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=512, nargs = '?', const = 128,
                        help='Batch Size')
    parser.add_argument('--num_workers', type=int, default=15, nargs = '?', const = 15,
                        help='Workers')
...
def get_latest_checkpoint(checkpoint_dir='checkpoints'):
    ...
    if args.pretrained == True: 
        checkpoints = [f for f in os.listdir(checkpoint_dir) if (f.startswith("ptepoch") and f.endswith('.pth'))]
